resource "signalfx_detector" "heartbeat" {
	...
	program_text = <<-EOF
		...
		signal = data('InstanceHealth', filter=filter('stat', 'mean') and filter('namespace', 'AWS/EBS') and ${module.filter-tags.filter_custom})
...
resource "signalfx_detector" "health" {
	...
	program_text = <<-EOF
		signal = data('EnvironmentHealth', filter=filter('namespace', 'AWS/EBS') and filter('stat', 'min') and ${module.filter-tags.filter_custom})${var.health_aggregation_function}.${var.health_transformation_function}(over='${var.health_transformation_window}')
...
resource "signalfx_detector" "5xx_error_rate" {
	...
	program_text = <<-EOF
		A = data('ApplicationRequests5xx', filter=filter('namespace', 'AWS/EBS') and filter('stat', 'sum') and ${module.filter-tags.filter_custom}){var.5xx_error_rate_aggregation_function}
		B = data('ApplicationRequestsTotal', filter=filter('namespace', 'AWS/EBS') and filter('stat', 'sum') and ${module.filter-tags.filter_custom}){var.5xx_error_rate_aggregation_function}
...
resource "signalfx_detector" "root_filesystem_usage" {
	...
	program_text = <<-EOF
		signal = data('RootFilesystemUtil', filter=filter('namespace', 'AWS/EBS') and filter('stat', 'min') and ${module.filter-tags.filter_custom}){var.root_filesystem_usage_aggregation_function}.${var.root_filesystem_usage_transformation_function}(over='${var.root_filesystem_usage_transformation_window}')
