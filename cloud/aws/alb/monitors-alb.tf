...
resource "signalfx_detector" "latency" {
	...
	program_text = <<-EOF
		...
		above_or_below_detector(signal, ${var.latency_threshold_critical}, lasting('${var.latency_aperiodic_duration}', ${var.latency_aperiodic_percentage})).publish('CRIT')
		above_or_below_detector(signal, ${var.latency_threshold_warning}, lasting('${var.latency_aperiodic_duration}', ${var.latency_aperiodic_percentage})).publish('WARN')
	EOF
